    ...
    public class Downloader
    {
        ...
        private static void SaveTMD(string saveDir, byte[] data)
        {
            try
            {
                ...
                File.WriteAllBytes(Path.Combine(saveDir, "title.tmd"), data);
            }
        ...
        public static async Task DownloadAsync(TitleData td, string saveFolder)
        {
            try
            {
                ...
                if (fake)
                    ...
                    {
                        var bytes = httpClient.GetByteArrayAsync(baseURL + "cetk").Result;
                        ticket = GetTicket(bytes);
                    }
                ...
                GetContentFilesAsync(tmd, saveDir, baseURL);
            }
            catch (Exception ex)
            {
        ...
        private static async Task<Tmd> GetTmdAsync(string baseUrl)
        {
            ...
            using (var httpClient = new HttpClient())
                try
                {
                    var bytearray = httpClient.GetByteArrayAsync(baseUrl + "tmd").Result;
                    tmd = new Tmd(bytearray);
                }
                catch (Exception we)
                {
        ...
        private static async Task GetContentFilesAsync(Tmd tmd, string saveDir, string baseUrl)
        {
            ...
            for (uint i = 0; i < contentCount; i++)
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    try
                    {
                        using (var response = httpClient.GetStreamAsync(new Uri(baseUrl + cidStr)).Result)
                        using (var fs = new FileStream(Path.Combine(saveDir, cidStr.ToUpper() + ".app"), FileMode.Create))
                           response.CopyTo(fs);
                    }
                    catch (Exception we)
                    {
                        string message = "ERROR! Could not download or save" + cidStr.ToUpper() + ".app";
                        throw new Exception(message, we);
                    }
                    //catch (IOException ioe)
                    //{
                    ...
                    try
                    {
                        using (var response = httpClient.GetStreamAsync(new Uri(baseUrl + cidStr + ".h3")).Result)
                        using (var fs = new FileStream(Path.Combine(saveDir, cidStr.ToUpper() + ".h3"), FileMode.Create))
                            response.CopyTo(fs);
                    }
                    catch (Exception we)
                    {
                        if (we.InnerException.ToString().Contains("404"))
                            Console.WriteLine(string.Format("WARNING: {0}.h3 not found, ignoring...", cidStr));
                        else
                        {
                            string message = "ERROR! Could not download or save" + cidStr + ".h3";
                            throw new Exception(message, we);
                        }
                    }
                    //catch (IOException ioe)
